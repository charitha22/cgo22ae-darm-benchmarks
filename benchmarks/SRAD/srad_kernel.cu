...
) 
{
  ...
  if(index_n >= 0 && index_n < rows * cols) north[ty][tx] = J_cuda[index_n]; 
  if(index_s >= 0 && index_s < rows * cols) south[ty][tx] = J_cuda[index_s];
  if ( by == 0 ){
  ...
  if(index_w >= 0 && index_w < rows * cols) west[ty][tx] = J_cuda[index_w];
  if(index_e >= 0 && index_e < rows * cols) east[ty][tx] = J_cuda[index_e];
...
) 
{
	...
	if(index_s >= 0 && index_s < rows * cols) south_c[ty][tx] = C_cuda[index_s];
	...
	if(index_e >= 0 && index_e < rows * cols ) east_c[ty][tx] = C_cuda[index_e];
